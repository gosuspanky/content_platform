...
from subsciptions.apps import SubsciptionsConfig
from subsciptions.views import (
    SubscriptionDeleteView,
    SubscriptionListView,
    CancelView,
    SuccessView,
    CreateCheckoutSessionView,
    BlogCheckoutPageView,
    StripeIntentView,
    stripe_webhook,
    SubscriptionCreateView,
)

app_name = SubsciptionsConfig.name


urlpatterns = [
    path(
        "subscription-create/<slug:slug>/",
        SubscriptionCreateView.as_view(),
        name="subscription_create",
    ),
    path(
        "subscription-cancel/<slug:slug>/",
        SubscriptionDeleteView.as_view(),
        name="subscription_delete",
    ),
    path(
        "subscription-list/",
        cache_page(60)(SubscriptionListView.as_view()),
