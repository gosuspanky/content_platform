...
class BlogListViewTest(SetupTestCase):
    ...
    def test_blog_list_view(self):
        Blog.objects.create(
            title="Blog 1", description="Description 1", published_on=True
        )
        Blog.objects.create(
            title="Blog 2", description="Description 2", published_on=True
        )
...
class BlogUpdateViewTest(SetupTestCase):
    ...
    def test_blog_update_view_unauthenticated_user(self):
        response = self.client.post(
            self.url, {"title": "Updated Title", "description": "Updated Description"}
        )
        self.assertEqual(response.status_code, 302)
