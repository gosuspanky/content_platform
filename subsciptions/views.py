...
from subsciptions.forms import SubscriptionForm
...
class BlogCheckoutPageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"blog": blog, "STRIPE_PUBLIC_KEY": settings.STRIPE_PUBLIC_KEY})
        return context
...
def stripe_webhook(request):
    ...
    if event.type == "checkout.session.completed":
        ...
        print(
            f"Payment succeeded for blog: {blog.title}. User phone: {request.user.phone}"
        )
...
class StripeIntentView(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            messages.success(
                request, "Оплата успешно проведена. Ваша подписка активирована!"
            )
...
class SubscriptionListView(ListView):
    ...
    def get_queryset(self):
        ...
        combined_blogs = (
            user_blogs | subscribed_blogs | user_unpublished_blogs
        ).distinct()
